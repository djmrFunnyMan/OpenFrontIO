import { Colord, extend, LabColor } from "colord";
...
export class ColorAllocator {
  ...
  assignColor(id: string): Colord {
    ...
    if (this.assigned.size > 50) {
      // More than 50 colors assigned just pick a random one for perf reasons,
      // as selecting a distinct color is O(n^2).
      selectedIndex = rand.nextInt(0, this.availableColors.length);
    } else {
      const assignedColors = Array.from(this.assigned.values());
      selectedIndex =
        selectDistinctColor(this.availableColors, assignedColors) ?? 0;
    }
...
export function selectDistinctColor(
  ...
  const assignedLabColors = assignedColors.map((c) => c.toLab());
  ...
  for (let i = 0; i < availableColors.length; i++) {
    ...
    const deltaE = minDeltaE(color.toLab(), assignedLabColors);
    if (deltaE > maxDeltaE) {
      maxDeltaE = deltaE;
      maxIndex = i;
    }
...
function minDeltaE(lab1: LabColor, assignedLabColors: LabColor[]) {
